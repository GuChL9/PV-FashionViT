...
\subsection{单种子逐项增广揭示的取舍}

\input{tables/augmentation_stages}

表\ref{tab:augmentation-stages}补充 seed 42 的四阶段控制变量结果。Center 到仅平移是位置鲁棒性的主要跃升：Large Shift Accuracy 从 $21.24\%$ 提升到 $74.03\%$，Robust Drop 从 65.43 个百分点缩小到 $-0.87$ 个百分点；代价是 Center Accuracy 从 $86.67\%$ 降到 $73.16\%$。但仅平移的 Rotation 和 Shift+Rotation Accuracy 仍只有 $41.68\%$ 和 $42.75\%$，说明见过各个位置并不会自动学会旋转不变性。
...
\begin{figure}[H]
  ...
  \caption{seed 42 的四阶段增广消融；柱形显示同一模型在四种测试协议下的准确率，结果用于解释变化方向而非替代五种子主实验}
