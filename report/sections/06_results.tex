...
\paragraph{证据边界}上述现象支持“训练覆盖、聚合方式和局部先验均可能影响鲁棒性”的解释，但不能构成三个因素各自贡献的因果分解。E4 同时引入平移、旋转和擦除，E6 又是在增强与 Mean Pooling 基础上加入 stem；加之正式实验只有一个随机种子，因此本节用“支持”而非“证明”描述机制判断。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{grid_accuracy_comparison.png}
  \caption{六组模型的 49 点 Grid Accuracy}
...
\begin{figure}[H]
  ...
  \caption{六组模型在连续随机旋转协议下的 Rotation Accuracy}
