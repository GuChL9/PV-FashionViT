...
后三个脚本对应收尾阶段的控制变量实验：前三个种子的逐项增强消融、固定 Center+CLS 条件下的位置编码消融，以及按完全相同欧氏位移半径分组的边缘裁剪分析。脚本会跳过已经存在 \texttt{evaluation.json} 的训练结果；若需从头复跑，可为前两个脚本添加 \texttt{-Force}。这些补充结果同样由原始 JSON/CSV 自动生成 LaTeX 表格与图片，避免手工转录误差。
